...
#define ANGLE 45
#define WIGGLEROOM .5#define MXP_IO_VOLTAGE (double)3.3f /* Alternately, 5.0f   */
...
class Robot:public frc::IterativeRobot
{
	...
	//Camera Tracking Config Values
	//The New GearBoxes are 1.5 times as fast so maybe just divide or add to the values?
	float HorzTune = .02;
	float FrdRevTune = .02; //Note: found a note with a value of 70 in the code

	float TurnTune = .663306452; //Couldnt find this in Donovans Code maybe someone else could
	...
	float Pixyx1Almost = 214; //Used
	float Pixyy1Almost = 95;
	float Pixyh1Almost = 51;
	float Pixyw1Almost = 29; //Used
	float Pixyx2Almost = 301; //Used
	...
	void AutonomousPeriodic() override
	{
		...
		switch(autonum)
		{
			...
			case 1:
				...
				break;
			case 2:
				frc::SmartDashboard::PutNumber("What the hell the Gyro's doing",ahrs->GetAngle());
				//Right Peg
				...
				{
					m_robotDrive.MecanumDrive_Cartesian(0, .5, (((ahrs->GetAngle() + 0) / 180) * 4), 0);
					frc::SmartDashboard::PutString("Feedback: ","Going Forward");
				}

				if((timer.Get() > 1.8) && (timer.Get()<2.7))
				{
					m_robotDrive.MecanumDrive_Cartesian(0, 0, (((ahrs->GetAngle() + 60) / 180) * 5), 0);
					frc::SmartDashboard::PutNumber("AutoAngle", ahrs->GetAngle());
					frc::SmartDashboard::PutString("Feedback: ","Turning");
				}
				if(timer.Get() > 2.7)
				{
					frc::SmartDashboard::PutString("Feedback: ","Targeting");
					PixyFunct();
					PixyDriveTakeover();
			...
			case 3:
				...
				if((timer.Get() > 1.9)&&(timer.Get()<2.9))
				{
					m_robotDrive.MecanumDrive_Cartesian(0, 0, (((ahrs->GetAngle() - 60) / 180) * 5), 0);
					frc::SmartDashboard::PutNumber("AutoAngle", ahrs->GetAngle());
					frc::SmartDashboard::PutString("Feedback: ","Turning");
				...
				if(timer.Get()>2.9)
				{
					...
					if(timer.Get() > 6)
					{
						frc::SmartDashboard::PutString("Feedback: ","Forward");
						if(!setTime)
						{
							setTime = true;
							autoTime = timer.Get();
			...
			case 4:
				...
				if(timer.Get() < 1)
				{
					m_robotDrive.MecanumDrive_Cartesian(0, .5 * TurnTune, 0, 0);
				}
				else
				{
					PixyFunct();
				...
				if(timer.Get() > 4)
				{
					...
					if((timer.Get() - autoTime) < 2)
					{
						m_robotDrive.MecanumDrive_Cartesian(0, .25 * TurnTune, (((ahrs->GetAngle() - 0) / 180) * kgyroManip), 0);
					}
					else
					{
						autonum = 0;
	...
	void PixyDriveTakeover()
	{
		...
		m_robotDrive.MecanumDrive_Cartesian(StrafePct, FrdRevPct, -testAng * TurnTune, 0);
	}
	;
	...
	void gearPegAngleTarget(float currentPeg,float xspeed,float yspeed)
	{
		...
		m_robotDrive.MecanumDrive_Cartesian(xspeed, yspeed, (((ahrs->GetAngle() - currentPeg) / 180) * kgyroManip * TurnTune), 0);
	}
	;
	...
	void TeleopPeriodic() override
	{
		...
		if(stick.GetRawButton(9)||stick.GetRawButton(8))
		{
